...
from utils import softmax,get_scale,change,Anchors
import numpy as np
import cv2
...
class tracker():
    ...
    def __init__(self, backbone='backbone.onnx', rpn_head='rpn.onnx'):
        self.backbone_session = ort.InferenceSession(backbone)
        self.head_session = ort.InferenceSession(rpn_head)
        ...
        self.score_size = (self.search_size - self.template_size) // \
            self.stride + 1
    ...
    def select_obj(self, img, bbox):
        print("IMAGE SIZE OUTPUT", np.array(img).shape)
        self.center_bbox =  np.array(
        ...
        # emb tesor
        print("Z SHAPE THIS", z.shape)
        self._z = self.backbone_session.run(
            None,
            {'data': z})
        print("Z: ",np.array(self._z).shape)
    ...
    def search_obj(self, x):
        ...
        print("X: ",np.array(self._z).shape)
    ...
    def get_subwindow(self, img, pos, model_sz, original_sz, avg_chans):
        ...
        print("PATH BEFORE: ", im_patch.shape)
        im_patch = im_patch.transpose(2, 0, 1)
        im_patch = im_patch[np.newaxis, :, :, :]
        im_patch = im_patch.astype(np.float32)
        print("PATH AFTER: ", im_patch.shape)
        return im_patch
